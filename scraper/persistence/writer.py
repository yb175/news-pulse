...
class DatabaseWriter:
    ...
    def ensure_unique_index(self):
        try:
            ...
            logger.warning(f"Could not ensure unique index on Cluster(title): {e}")
    ...
    def get_or_create_cluster(self, title: str, summary: str = "") -> str:
        ...
        try:
            ...
            logger.warning(f"Conflict detected: Cluster '{normalized_title}' was created concurrently. Retrying lookup...")
            try:
                with self._get_connection() as conn:
                    with conn.cursor() as cur:
                        cur.execute('SELECT "id" FROM "Cluster" WHERE "title" = %s;', (normalized_title,))
